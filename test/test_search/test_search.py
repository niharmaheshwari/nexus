...
class TestSnippetSnapshot(TestCase):
    ...
    def setUp(self):
        ...
        self.stub_client = Stubber(self.snippet_manager._db_client)
    ...
    def test_get_snippet_snapshots_success(self):
        ...
        'timed_out': False,
        'took': 11}
        with mock.patch('src.manager.snippet_snapshot_manager.OpenSearch.search', return_value=es_response):
            rsp, err = self.snippet_snapshot_manager.search_by_string('binary search','nm3223@columbia.edu')
    ...
    def test_get_snippet_snapshots_fail(self, mock):
    ...
    @mock.patch('src.manager.search_manager.SnippetSnapshotManager.search_by_string', return_value=(None,MessageFormat().error_message("error")))
    def test_search_failure(self, mock):
        rsp = self.search_manager.search_general("search string", "tk2892@columbia.edu")
        self.assertTrue(rsp['error'])
    
    @mock.patch('src.manager.search_manager.SnippetSnapshotManager.search_by_string', return_value=([], None))
    def test_search_general_happy_path_no_snippets(self, mock):
    ...
    return_value=([SnippetSnapshot('id1',['binary'], 'bs', 'python'),SnippetSnapshot('id2',['tags'], 'desc', 'c++')], None))
    @mock.patch('src.manager.search_manager.SnippetManager.get_snippets', return_value=MessageFormat().error_message("error"))
    ...
    def test_search_happy_path(self, mock1):
        ...
        with mock.patch('src.manager.search_manager.SnippetManager.get_snippets', return_value=MessageFormat().success_message(data=data)):
            rsp = self.search_manager.search_general("string", "email")
        self.assertFalse(rsp['error'])

    #TODO if time: add a test for snippet_manager.get_snippets
