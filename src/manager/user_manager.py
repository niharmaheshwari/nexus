...
from src.utilities.authentication_utils import *
...
class UserManager():
    ...
    def confirm_signup(self, email, otp):
        ...
        try:
            ...
            return MessageFormat().error_message("User is already confirmed")
        except Exception as err:
            return MessageFormat().error_message("Unknown error: {}".format(str(err)))
        return MessageFormat().success_message(data=None)
    ...
    def generate_new_token(self, refresh_token_details):
        ...
        try:
            ...
            return MessageFormat().error_message("Invalid email/token")
        except self._cognito_client.exceptions.UserNotConfirmedException:
    ...
    def get_user_details(self, token, email):
        ...
        try:
            ...
            for field in user_attributes:
                user_handler_map[field["Name"]](field["Value"])
            return user
            # return MessageFormat().success_message(data=response["UserAttributes"])
        except self._cognito_client.exceptions.UserNotFoundException:
